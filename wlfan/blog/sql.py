...
def searchByTitle(kw):
    s = "SELECT * FROM blog WHERE b_title like'%" + kw + "%' ORDER BY b_id"
    return s
...
def editblog(id, title, tag, pre, text):
    s = "UPDATE blog SET b_title = '" + title + "',b_tag='" + tag + "',b_pre='" + pre + "',b_text='" + text + "' WHERE b_id=" + str(
        id)
    return s
...
def getblog(b_id):
    s = "SELECT *  FROM blog  WHERE b_id='" + b_id + "'"
    return s


# 数据库连接函数
def sql_dict(sql):
    connection = pymysql.connect(host='localhost', port=3306, user='root', passwd='root', db='sandwinds',
                                 charset='utf8')
